...
const ContactMe = () => {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    if(
      ...
      setOutput({message: "Missing required fields.", error:true, loading: false})
      return;
    }
    if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(inquiry.email) === false) {
      setOutput({message: "Invalid email format.", error:true, loading: false})
      return;
    }
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <textarea
          ...
          className="w-full p-2 backdrop-blur-xl text-white text-xl h-40 resize-none outline-none border border-neutral-800 sm:border-neutral-700 bg-neutral-900/60 rounded-xl"
        ></textarea>
